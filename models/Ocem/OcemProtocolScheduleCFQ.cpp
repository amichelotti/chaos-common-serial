		...
		for(i=slave_queue_sorted.begin();i!=slave_queue_sorted.end();i++){
			...
			if(size>0){
				...
				if(ret>0){
					...
					DPRINT("[%s %i] command \"%s\" ERROR(errs %d) , ret=%d timeo=%d",serdev,i->first,(char*)cmd.buffer.c_str(),write_queue->req_bad,ret,timeo);
					if((cmd.retry<3) && (cmd.buffer != "SL") && (cmd.buffer!="SA")){
						ERR("[%s,%d] scheduled for retry command \"%s\", retries %d, tot bad %d, in queue %d",serdev,i->first,(char*)cmd.buffer.c_str(),cmd.retry,write_queue->req_bad,write_queue->size());
...
int OcemProtocolScheduleCFQ::registerSlave(int slaveid){
	...
	if(slave_queue.find(slaveid)==slave_queue.end()){
		...
		slave_queue.insert(std::pair<int,std::pair<OcemProtocolScheduleCFQ::OcemData*,OcemProtocolScheduleCFQ::OcemData* > >(slaveid,std::pair<OcemProtocolScheduleCFQ::OcemData*,OcemProtocolScheduleCFQ::OcemData*>(d,s)));
		slave_queue_sorted.push_back(std::pair<int,std::pair<OcemProtocolScheduleCFQ::OcemData*,OcemProtocolScheduleCFQ::OcemData* > >(slaveid,std::pair<OcemProtocolScheduleCFQ::OcemData*,OcemProtocolScheduleCFQ::OcemData*>(d,s)));
	} else {
		pthread_mutex_unlock(&mutex_buffer);
