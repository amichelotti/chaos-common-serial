...
void *OcemProtocolBuffered::runSchedule() {
  ...
  while (run) {
    while (queue_ocem_write.empty() != 0 ||
      ...
      DPRINT("[%d] scheduling WRITE (%d/%d/%d) , cmd queue %d, SENDING command \"%s\", timeout %d, issued %llu us ago",
      ...
      for (std::vector<std::string>::iterator cmd_it = current_cmd.command_list.begin();
        ...
        if ((ret = OcemProtocol::select(current_cmd.serial_id,
          ...
          queue_ocem_write.req_ok++;
          DPRINT("[%d] command ok queue lenght %d", current_cmd.serial_id, write_queue->size());
        }
      }

      if ((queue_ocem_write.size() < MAX_WRITE_QUEUE)) {
        pthread_cond_signal(&queue_ocem_write.awake);
      }

      consume_message_number_to_send--;
    }

    consume_message_number_to_send = 2;
    do {
...
int OcemProtocolBuffered::poll(int& slave_id,
  ...
  if (!queue_ocem_read.empty()) {
    ...
    DPRINT("[%d] poll queue %d returned \"%s\"", slave_id, read_queue->size(), req.buffer.c_str());
...
int OcemProtocolBuffered::select(int& slave_id,
  ...
  if (queue_ocem_write.size() >= MAX_WRITE_QUEUE) {
    DPRINT("[%d] WAIT for cmd \"%s\" queue reduce %d", slave_id, command, size);
    if (wait_timeo(&queue_ocem_write.awake,
                   &queue_ocem_write.qmutex,
                   0) < 0) {
      DERR("[%d] too many commands on queue %d", slave_id, size);
      return 0;
    }
  }

  DPRINT("[%d] pushing command \"%s\"", slave_id, command);
