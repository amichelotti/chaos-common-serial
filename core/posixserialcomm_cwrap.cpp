...
extern "C" {
  ...
  static common::serial::AbstractSerialChannel* id2handle[MAX_HANDLE]={0};
  /*
  ...
  */
  pserial_handle_t popen_serial(int internal_buffering,const char*serdev,int baudrate,int parity,int bits,int stop,bool hw){
    ...
    common::serial::AbstractSerialChannel*p = new common::serial::PosixSerialComm(serdev,baudrate,parity,bits,stop,hw,internal_buffering,internal_buffering);
  ...
  int pclose_serial(pserial_handle_t _h){
    ...
    common::serial::AbstractSerialChannel* h = id2handle[_h];
    if(h){
      common::serial::AbstractSerialChannel *p =( common::serial::AbstractSerialChannel *)h;
      ret = p->deinit();
      delete p;
      id2handle[_h] = 0;
      return ret;
